...
def clear_data():
    ...
    if data.get("confirm") != "DELETE":
        return jsonify({"ok": False, "error": "Confirmation required. Type DELETE."}), 400

    filters = {}
    for key in ["firm", "staff", "pc", "sku", "status", "batch"]:
        val = str(data.get(key, "ALL")).strip()
        if val and val != "ALL":
            filters[key] = val

    where, params = build_where(filters)
    clear_activity = bool(data.get("clear_activity", False))

    conn = db()
    cur = conn.cursor()
    cur.execute(f"SELECT COUNT(*) FROM orders {where}", params)
    deleted_orders = cur.fetchone()[0]
    cur.execute(f"DELETE FROM orders {where}", params)
    ...
    if clear_activity:
        ...
        ap = []
        if filters.get("firm"):
            aw.append("firm_name=?"); ap.append(filters["firm"])
        if filters.get("staff"):
            aw.append("staff_name=?"); ap.append(filters["staff"])
        if filters.get("pc"):
            aw.append("pc_name=?"); ap.append(filters["pc"])
        awhere = (" WHERE " + " AND ".join(aw)) if aw else ""
        cur.execute(f"SELECT COUNT(*) FROM staff_activity {awhere}", ap)
        deleted_activity = cur.fetchone()[0]
        cur.execute(f"DELETE FROM staff_activity {awhere}", ap)

    conn.commit()
    conn.close()
    return jsonify({"ok": True, "deleted_orders": deleted_orders, "deleted_activity": deleted_activity})
...
<label><b>Status:</b></label><select id="statusFilter"><option value="ALL">ALL</option><option value="P">Pending</option><option value="R">Ready</option><option value="N">Not Found</option><option value="S">Uncomplete</option></select>
...
<button onclick="clearSelected(false)" class="dangerBtn">Clear Selected Orders</button>
<button onclick="clearSelected(true)" class="dangerBtn">Clear Selected + Activity</button>
...
function params(){let p=new URLSearchParams();let f=document.getElementById('firmFilter').value, st=document.getElementById('staffFilter').value, sk=document.getElementById('skuFilter').value, status=document.getElementById('statusFilter').value;if(f!=='ALL')p.set('firm',f);if(st!=='ALL')p.set('staff',st);if(sk!=='ALL')p.set('sku',sk);if(status!=='ALL')p.set('status',status);return p}
...
async function clearSelected(activity){let f=document.getElementById('firmFilter').value, st=document.getElementById('staffFilter').value, sk=document.getElementById('skuFilter').value, status=document.getElementById('statusFilter').value;let scope=`Firm=${f}, Staff=${st}, SKU=${sk}, Status=${status}`;let c=prompt(`DANGER: Selected data delete hoga. Scope: ${scope}. Confirm ke liye DELETE type karo:`);if(c!=='DELETE')return;let res=await fetch('/clear-data',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({firm:f,staff:st,sku:sk,status:status,clear_activity:activity,confirm:'DELETE'})});let d=await res.json();alert(d.ok?`Deleted orders: ${d.deleted_orders}, activity: ${d.deleted_activity}`:'Error: '+d.error);loadData()}
['firmFilter','staffFilter','skuFilter','statusFilter'].forEach(id=>document.getElementById(id).addEventListener('change',renderData));document.getElementById('searchBox').addEventListener('input',renderData);loadData();setInterval(loadData,3000);
</script></body></html>
    """)
